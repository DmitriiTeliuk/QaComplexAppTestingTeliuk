...
class StartPage(BasePage):
    ...
    def log_in(self, user_name, user_password):
        ...
        sign_in_button = self.wait_until_find_visible_and_clickable(by=By.XPATH, value=self.constance.BUTTON_SIGN_IN_XPATH)
    ...
    def get_new_user_name_field_alert(self):
        alert = self.wait_until_find_element(by=By.XPATH, value=self.constance.ALERT_NEW_USER_NAME_XPATH)
        assert alert.is_displayed()
        self.log.info("'new_user name field alert' is displayed")
        return alert.text

    def get_new_user_email_field_alert(self):
        alert = self.wait_until_find_element(by=By.XPATH, value=self.constance.ALERT_NEW_USER_EMAIL_XPATH)
        assert alert.is_displayed()
        self.log.info("'new_user email field alert' is displayed")
        return alert.text

    def get_new_user_password_field_alert(self):
        alert = self.wait_until_find_element(by=By.XPATH, value=self.constance.ALERT_NEW_USER_PASSWORD_XPATH)
        assert alert.is_displayed()
